...
class Agent():
    def __init__(self,action_space,replay_memory_len,replay_memory_max_len,init_temp,alpha, debug = False):
        self.replay_memory = ReplayMemory(replay_memory_max_len)
        self.starting_mem_len = replay_memory_len
        self.action_space = action_space
        self.temperature = init_temp
        self.temperature_decay = 0.01
        self.temperature_min = 0.02
        self.gamma = 0.99
        self.alpha = alpha
        self.total_steps = 0
        self.num_trained = 0
    ...
    def select_action(self,state):
        # SOFTMAX EXPLORATION
        action_output = self.model.predict(state)
        soft_probs = tf.nn.softmax(action_output[0]/self.temperature,axis=-1)
        soft_probs = np.array(soft_probs)
        a_index = np.random.choice(np.arange(3),p = soft_probs)
